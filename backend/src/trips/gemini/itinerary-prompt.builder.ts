...
export function buildItineraryPrompt(
  ...
  return [
    ...
    `- 활동 시간대: ${dto.activity_time_start} ~ ${dto.activity_time_end}`,
    `- 동반인: ${dto.companion}`,
    ...
    `4. time은 24시간제 HH:MM 형식으로, 반드시 활동 시간대(${dto.activity_time_start} ~ ${dto.activity_time_end}) 범위 안에서만 배정해라. 이 범위를 벗어나는 활동을 만들지 마라. duration_minutes는 분 단위 정수, estimated_cost는 1인 기준 예상 비용(현지 통화 정수, 확실하지 않으면 0)으로 채워라.`,
    '5. location에는 지오코딩이 가능하도록 구체적인 장소명 또는 주소를 적어라.',
    `6. 예산 수준(${dto.budget_level}), 동반인(${dto.companion}), 취향(${preferencesText})을 활동 선택에 실제로 반영해라(예: 동반인이 '아이'면 아이 동반에 적합한 활동/이동 강도로, '연인/배우자'면 둘이 즐길 수 있는 활동 위주로 구성).`,
