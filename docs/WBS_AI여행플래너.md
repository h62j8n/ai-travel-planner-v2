...
| 2.2 | Gemini API 연동 | 최초 생성/전체 재생성/일자별 활동 교체 프롬프트 설계, 일자별 동선 생성 로직 | Gemini API |
...
| 2.6 | 결과 표시 화면(일정표) | 일자별 카드 UI, "전체 재생성" + day별 "활동 재생성" 버튼 | React, MUI |
| 2.7 | POST /api/trips/{id}/regenerate 구현 | 전체 재생성 API, 캐시 무시, 새 trip_id 반환 | NestJS |
| 2.8 | PATCH /api/trips/{id}/regenerate-day 구현 | 일자별 활동 교체 API, 다른 day 정보를 프롬프트에 포함해 중복 방지 | NestJS |

**완료 기준**: 폼 입력 → AI 생성 → 결과 표시까지 에러 없이 동작 / 동일 조건 재요청 시 캐시 히트 확인 / 전체 재생성 → 새 trip 생성 확인 / 일자별 활동 교체 → 해당 day만 새 활동, 다른 day 유지 확인 / 일자별 활동 교체 시 다른 day와 중복 없음 확인
